#!/bin/sh
# Andbo stub agent — a zero-cost PROOF FIXTURE, not a real coding agent.
#
...
if [ -n "$key" ]; then
	...
	# Defense-in-depth: Andbo must redact this value from the saved session.
	echo "stub-agent: RAWKEY=${key} (this MUST appear redacted in the session)"
else
	echo "stub-agent: NO injected API key"
fi

printf 'stub agent ran for task: %s\n' "$task" > andbo-stub-agent-output.txt
echo "stub-agent: wrote andbo-stub-agent-output.txt"
