...
# — that is the point: it proves AgentBox runs a baked-in agent the host cannot.
COPY stub-agent.sh /usr/local/bin/agentbox-stub-agent
RUN chmod 0755 /usr/local/bin/agentbox-stub-agent \
