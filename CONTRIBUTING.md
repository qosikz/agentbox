# Contributing to Andbo

Thanks for helping build Andbo — a secure, CLI-first sandbox for AI coding
...
- **Use it and report back** — open a [bug report](../../issues/new/choose) with
  your `andbo version`, OS, and container engine. Real-world coverage is what
...
```bash
git clone https://github.com/qosikz/andbo
cd andbo
...
5. Appreciated: a worked `examples/agents/<name>.Dockerfile` + an
   `examples/andbo.<name>.yaml`, and a row in the README adapter list.
...
`TYPE=0 HOLD=0 ANDBO="$PWD/bin/andbo" demo/<name>-demo.sh`, then record with
**asciinema 3.x** (`--headless --window-size 92x30`) and render with `agg`. One
rule for security demos: **never echo a secret to live stdout** — `andbo exec`
...
[adv]: https://github.com/qosikz/andbo/security/advisories/new
