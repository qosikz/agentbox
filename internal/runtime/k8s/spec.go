...
func (s JobSpec) EnforcementNotes() []string {
	return []string{
		...
		"the NetworkPolicy must exist for the WHOLE lifetime of the pod, and it must go on SAYING what it said when you reviewed it. It carries no ownerReference, so it is neither garbage-collected with the Job nor protected from removal: deleting it (or pruning by label while a run is in flight) restores full egress to a running agent immediately. Deletion is the loud route and not the only one. A NETWORKPOLICY HAS NO IMMUTABLE SPEC FIELDS — ValidateNetworkPolicyUpdate re-runs the same spec validation a create does and pins nothing in it, where the Job at least freezes its pod template — so anyone who can update it rewrites the live object rather than removing it. (Its METADATA is a different matter, and the distinction is the reason the edit is quiet rather than a technicality: ValidateObjectMetaUpdate does run ValidateImmutableField, on name, namespace, uid, creationTimestamp, deletionTimestamp and deletionGracePeriodSeconds. The identity you reviewed is pinned; the behaviour is not.) Three edits reach the same end, and they are worth naming separately because a reader watching for one will miss the others: ADDING AN EGRESS RULE, where an empty one is the widest hole available since upstream reads an empty `to` as every destination and an empty `ports` as every port; DROPPING \"Egress\" FROM POLICYTYPES, since a NetworkPolicy restricts only the directions it names, and emptying the list outright is no safer because the API server defaults a policy with no egress rules to [Ingress] alone; or REPOINTING PODSELECTOR at labels no pod carries. None of the three deletes anything. The object is STILL there, still named for the run, still in the pod's namespace, still carrying the run's labels — which is exactly what those six immutable metadata fields guarantee — so it reads as present and bound in `kubectl get networkpolicy` while the agent has egress, and that is why this is quieter than the deletion above rather than merely equivalent to it. Render refuses TWO of the three at manifest time — deniesEveryDirection for the policyTypes half and bindsPolicyToPod for the selector — and the third is refused not by a guard but by construction, since JobSpec has no field that renders an ingress or egress rule and the NetworkPolicy's own field set is closed by test. Manifest time is the only time a renderer can refuse anything at all; namespace RBAC on networkpolicies — the update verb as much as delete — is what defends the live object, and a persistent namespace-wide default-deny that does not depend on this object is the durable backstop. Read the deny-all as a property of the manifest at apply time, not of the run",
		...
		"backoffLimit 0 and restartPolicy Never are one control on two axes and are only correct together: backoffLimit bounds how many replacement PODS the Job controller creates after a failure, while restartPolicy decides whether the KUBELET restarts the container in place. The second is not a hole in the first — the Job controller does count in-place container restarts against backoffLimit, and at 0 the first one fails the Job — but it acts only after the kubelet has already restarted the container, so under OnFailure the agent begins a second time on the half-written workspace of the first, and is then killed along with its logs. That is ONE MORE START and not an unbounded number, which is the honest size of the difference and still reason enough for an agent that commits and pushes — the more so because that second attempt is not cut short instantly: the Job controller deletes the pod with no grace-period override, so it gets the pod's terminationGracePeriodSeconds, which this renderer never sets and which therefore defaults to 30 seconds, between SIGTERM and SIGKILL. Long enough to commit and push, and for a short task long enough to finish. What you are left holding is a Job in Failed/BackoffLimitExceeded whether or not that second attempt succeeded, with the pod and its logs already deleted, so you CANNOT TELL FROM THE JOB WHETHER THE AGENT COMMITTED OR PUSHED — check the repository, not the Job. Neither field is at-most-once execution: node failure, preemption, or pod deletion can start the same run a second time, and nothing stops the same manifest being applied twice. Agent side effects must be idempotent or keyed by a run ID. Once the manifest is applied the API server treats the two halves oppositely. RESTARTPOLICY CANNOT BE CHANGED ON THIS JOB AT ALL: it rides in spec.template, and every branch of the update path ends in an immutability check on the template — unsuspended it is immutable outright, and while suspended the only exemptions are container resources and the scheduling directives (nodeSelector, tolerations, node affinity, labels, annotations, scheduling gates), none of which reaches restartPolicy. BACKOFFLIMIT IS THE HALF NOTHING PINS: it appears in none of the ValidateImmutableField calls that path makes, so anyone who can update the Job raises it, the controller compares the new value on its next sync, and the replacement pod this manifest refuses IS ONE EDIT AWAY FOR AS LONG AS THE AGENT IS ALIVE. ONLY THE JOB'S OWN END CLOSES THAT WINDOW, which is worth knowing before you go looking for a mitigation: a Job already carrying a Complete or Failed condition is skipped by the controller before it reads the retry budget, so raising backoffLimit after a BackoffLimitExceeded restarts nothing. Read the no-retry claim as a property of the manifest at apply time, not of the run",
		"parallelism 1 and completions 1 say this Job asks for one pod, which is a bound on what the JOB SCHEDULES and not a count of how many times the agent runs. Read it alongside the note above: the same run can still be started twice by the cluster, and nothing here stops a second Job being applied from the same manifest. Once the Job exists, COMPLETIONS IS THE ONLY ONE OF THE TWO THE CLUSTER HOLDS, and by a chain rather than by the field being special: the update path rejects a change to completions unless the Job is Indexed, this Job is not (the renderer emits no completionMode, and the API server defaults an absent one to NonIndexed), and completionMode is itself immutable — so IT CANNOT BECOME INDEXED LATER. parallelism is not held, and PARALLELISM IS A RESTART SWITCH AND NOT A NO-OP. Raising it buys no CONCURRENT second pod while completions is 1, because the controller wants completions-minus-successes pods and only then caps that by parallelism — but LOWERING it to 0 is an equally legal update, and the controller then deletes the running pod WITHOUT COUNTING A FAILURE AGAINST BACKOFFLIMIT 0, because it strips the pod's job-tracking finalizer before issuing the delete and every counting path skips a finalizer-less pod. The Job is left alive with no pod, and raising parallelism back STARTS THE AGENT OVER — on the workspace the first attempt half-wrote, after whatever it already committed or pushed. Do both quickly and the replacement is created while the original is still inside its termination grace period, giving two agents on one repository with one set of credentials, because the controller subtracts terminating pods from what it creates only under a podReplacementPolicy this renderer does not emit. So the rendered parallelism 1 is a statement of intent the cluster is not enforcing, and completions is what actually holds the count down. All of it concerns THIS Job object: DELETE-AND-RECREATE IS NOT AN UPDATE, and neither is a second Job from the same manifest",
		"the pod declares ONE agent container, plus — with workspaceTransport=image only — ONE init container that copies the workspace in. That is a bound on what the MANIFEST DECLARES, and it is worth reading beside the two notes above because it is the one whose apply-time half does hold — though not for the reason it first appears to. THE CONTAINER LISTS RIDE IN SPEC.TEMPLATE, and the template is NOT simply frozen: while the Job is suspended the update path exempts container resources, and a separate exemption lifts the scheduling directives INCLUDING THE TEMPLATE'S LABELS, which are what the NetworkPolicy above binds to. What holds container-list MEMBERSHIP is narrower and stronger: the resources exemption copies the new values across only when the container counts match and the names line up index-for-index, then compares the whole pod spec for equality, so a list that grew, shrank, or was renamed is refused in every branch. A live pod is stricter still, admitting only the containers' and init containers' IMAGE. So nobody edits a container into a live Job the way they can raise backoffLimit or drop parallelism. What no manifest can hold is the POD. AN EPHEMERAL CONTAINER IS ADDED THROUGH THE POD'S OWN EPHEMERALCONTAINERS SUBRESOURCE rather than through the template — `kubectl debug` is the usual route — so it never meets that immutability at all: anyone holding that permission attaches a container of their own choosing to the RUNNING pod, sharing its network namespace and therefore whatever egress the NetworkPolicy leaves it, and sharing its process namespace too when they pass a target (which the container runtime has to implement — containerd and CRI-O do; upstream calls the result undefined where it is unsupported). It CANNOT BE REMOVED ONCE ADDED, and it is not visible in the manifest you reviewed. Nothing rendered here can refuse it; namespace RBAC on pods/ephemeralcontainers is where it is refused. Read the one-container claim as a property of the manifest at apply time, not of the run",
		"imagePullPolicy Always makes the kubelet re-resolve the image REFERENCE at the registry on every start, so a node cannot go on serving whatever it resolved for that reference earlier. That is a FRESHNESS control and not tamper detection: once the reference resolves to a digest the node already stores, the container runtime reuses the cached layers and nothing re-verifies them, so a node with a compromised image store serves the same bytes under Always as under IfNotPresent. Re-resolving a mutable tag can also return different bytes on each start, so this is an identity guarantee only when the reference is a digest. The pull is the kubelet's, from whatever registry and credentials the node has: this manifest does not sign, verify, or admit the image, and the NetworkPolicy does not restrict the pull. The policy itself does survive being applied — imagePullPolicy lives in the pod template, so it is held by the SAME TEMPLATE IMMUTABILITY that holds restartPolicy — but what no immutability can hold is what the registry serves for that reference on the next pull",
		...
		"activeDeadlineSeconds is when the cluster BEGINS ending the run and NOT WHEN THE AGENT STOPS. The Job controller deletes the pod through a call that takes no delete options at all, so it cannot ask for a shorter shutdown: the agent goes on running for UP TO the pod's terminationGracePeriodSeconds — which this renderer never sets, so the 30-second default — between SIGTERM and SIGKILL. That is a ceiling and not a duration: an agent that handles SIGTERM exits at once, while one that ignores it burns the whole window, which is the common case for a PID 1 with no handler. A commit or push already under way finishes only if it fits in what is left, and one that does not is SIGKILLED MID-FLIGHT — a half-written push or a repository left holding an index.lock, which is worse than either finishing or not starting. What you are left holding is a Job in Failed/DeadlineExceeded with the pod and its logs deleted, so a DeadlineExceeded Job IS NOT EVIDENCE THAT THE AGENT DID NOTHING — check the repository, not the Job. The budget measures one continuously-active period rather than the Job's lifetime: it runs from status.startTime, the controller does not evaluate it while a Job is suspended, and resuming resets startTime to the moment of the resume. A SUSPEND AND RESUME HANDS THE RUN A FRESH FULL BUDGET AND STARTS THE AGENT OVER, and none of it counts as a failure: suspending deletes the pod, but THE CONTROLLER STRIPS THAT FINALIZER BEFORE IT DELETES, so neither counting path ever sees the pod and backoffLimit 0 is never reached. The Job takes a JobSuspended condition and stays unfinished; resuming resets status.startTime to that moment and creates a new pod. Anyone who can update the Job therefore holds both an unbounded wall-clock extension and a way to re-run an agent that has already committed or pushed. (An earlier version of this note said the opposite — that a suspend kills the run, because a deleted pod counts as a failure against backoffLimit 0. That is true of a pod deleted by anyone ELSE; it is false for the controller's own deletions, which are exactly the ones a suspend causes.) Raising the deadline is simply the more direct of the two: activeDeadlineSeconds is MUTABLE ON A LIVE JOB — the update validation pins selector, completionMode, podFailurePolicy, backoffLimitPerIndex, managedBy and successPolicy, and not this one (it separately constrains the pod template and completions, so read that list as what it is: the immutable SPEC FIELDS, not everything the update path checks, and it also freezes the basic/gang variant of spec.scheduling) — so the same permission that could suspend the Job can instead just raise the number, without even restarting the agent, and the budget in the manifest you reviewed is the budget at apply time rather than for the life of the run. Enforcement is entirely the cluster's — nothing in Andbo supervises a pod — so a Job reconciled by another controller through managedBy, which the built-in controller skips, has no deadline applied at all",
