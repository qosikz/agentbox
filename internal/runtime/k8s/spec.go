...
func (s JobSpec) EnforcementNotes() []string {
	return []string{
		...
		"the pod declares ONE agent container, plus — with workspaceTransport=image only — ONE init container that copies the workspace in. That is a bound on what the MANIFEST DECLARES, and it is worth reading beside the two notes above because it is the one whose apply-time half mostly holds: THE CONTAINER LISTS RIDE IN SPEC.TEMPLATE, which every branch of the Job update path refuses to change, so unlike backoffLimit and parallelism nobody edits a container into a live Job. What no manifest can hold is the POD. AN EPHEMERAL CONTAINER IS ADDED THROUGH THE POD'S OWN EPHEMERALCONTAINERS SUBRESOURCE rather than through the template — `kubectl debug` is the usual route — so it never meets that immutability at all: anyone holding that permission attaches a container of their own choosing to the RUNNING pod, sharing its network namespace and therefore whatever egress the NetworkPolicy leaves it, and sharing its process namespace too when they pass a target. It CANNOT BE REMOVED ONCE ADDED, and it is not visible in the manifest you reviewed. Nothing rendered here can refuse it; namespace RBAC on pods/ephemeralcontainers is where it is refused. Read the one-container claim as a property of the manifest at apply time, not of the run",
