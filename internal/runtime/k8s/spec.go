...
func (s JobSpec) EnforcementNotes() []string {
	return []string{
		...
		"backoffLimit 0 and restartPolicy Never are one control on two axes and are only correct together: backoffLimit bounds how many replacement PODS the Job controller creates after a failure, while restartPolicy decides whether the KUBELET restarts the container in place. The second is not a hole in the first — the Job controller does count in-place container restarts against backoffLimit, and at 0 the first one fails the Job — but it acts only after the kubelet has already restarted the container, so under OnFailure the agent begins a second time on the half-written workspace of the first, and is then killed along with its logs. That is ONE MORE START and not an unbounded number, which is the honest size of the difference and still reason enough for an agent that commits and pushes — the more so because that second attempt is not cut short instantly: the Job controller deletes the pod with no grace-period override, so it gets the pod's terminationGracePeriodSeconds, which this renderer never sets and which therefore defaults to 30 seconds, between SIGTERM and SIGKILL. Long enough to commit and push, and for a short task long enough to finish. What you are left holding is a Job in Failed/BackoffLimitExceeded whether or not that second attempt succeeded, with the pod and its logs already deleted, so you CANNOT TELL FROM THE JOB WHETHER THE AGENT COMMITTED OR PUSHED — check the repository, not the Job. Neither field is at-most-once execution: node failure, preemption, or pod deletion can start the same run a second time, and nothing stops the same manifest being applied twice. Agent side effects must be idempotent or keyed by a run ID. Once the manifest is applied, ONLY ONE OF THE TWO SURVIVES THE APPLY. restartPolicy Never sits inside spec.template, and the update path REFUSES TO CHANGE THE TEMPLATE AT ALL while the Job is not suspended, so nobody turns the kubelet's in-place restarts back on afterwards. backoffLimit has no such protection — it is absent from every immutable-field check that path makes — so anyone who can update the Job RAISES IT ON A RUNNING JOB, and the controller compares the new value on its next sync: the replacement pod this manifest refuses is one edit away for as long as the agent is alive. Its only bound is the Job's own end, and it is worth knowing before you go looking for a mitigation — a Job already carrying a Failed condition is skipped by the controller before it reads the spec at all, so RAISING IT AFTER A BACKOFFLIMITEXCEEDED RESTARTS NOTHING. Read the no-retry claim as a property of the manifest at apply time, not of the run",
		"parallelism 1 and completions 1 say this Job asks for one pod, which is a bound on what the JOB SCHEDULES and not a count of how many times the agent runs. Read it alongside the note above: the same run can still be started twice by the cluster, and nothing here stops a second Job being applied from the same manifest. Of the two fields, only completions is actually held once the Job exists, and by a chain worth knowing rather than by the field being special: the update path rejects a change to completions unless the Job is Indexed, this Job is not (the renderer emits no completionMode, and the API server defaults an absent one to NonIndexed), and completionMode is itself immutable — so IT CANNOT BECOME INDEXED LATER. parallelism is not held and can be raised on a running Job; what saves the property is that raising it buys no second pod while completions stays 1, because the controller asks for completions-minus-succeeded pods and only then caps that number by parallelism. The one-pod bound therefore RESTS ON COMPLETIONS ALONE, and the rendered parallelism 1 is a statement of intent the cluster is not enforcing. That is immutability of THIS Job object and nothing wider: DELETE-AND-RECREATE IS NOT AN UPDATE, and neither is a second Job from the same manifest",
