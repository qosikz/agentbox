...
func (s JobSpec) EnforcementNotes() []string {
	return []string{
		...
		"imagePullPolicy Always re-resolves the image REFERENCE on every start, so a node cannot silently serve a stale or tampered cached layer for it — but re-resolving a mutable tag can return different bytes each time, so it is an identity guarantee only when the reference is a digest. The pull is the kubelet's, from whatever registry and credentials the node has: this manifest does not sign, verify, or admit the image, and the NetworkPolicy does not restrict the pull",
