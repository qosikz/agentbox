...
func (s JobSpec) EnforcementNotes() []string {
	return []string{
		...
		"backoffLimit 0 and restartPolicy Never are one control on two axes and are only correct together: backoffLimit bounds how many replacement PODS the Job controller creates after a failure, while restartPolicy decides whether the KUBELET restarts the container in place. An in-place restart is not a pod failure, so it never consumes backoffLimit — under restartPolicy OnFailure the agent would be restarted again and again on the same workspace while the Job still read backoffLimit 0. Neither is at-most-once execution: node failure, preemption, or pod deletion can start the same run a second time, and nothing stops the same manifest being applied twice. Agent side effects must be idempotent or keyed by a run ID",
