...
func (r *Root) cmdRun(ctx context.Context, args []string) error {
	...
	if o.repo != "" && looksLikeRepo(o.repo) {
		if o.dryRun {
			...
			dest := filepath.Join(base, ".agentbox", "work", rec.Session.ID)
	...
	// 8. For real runs, materialize a disposable, sanitized workspace copy
	//    (denied files like .env are excluded; .git is restored so diffs work).
	execRoot := root
	var repo *git.Repo
	if !o.dryRun {
		if o.repo != "" && looksLikeRepo(o.repo) {
			execRoot = root // the clone IS the workspace
		} else {
	...
	if repo != nil && !o.dryRun {
		...
		if files, ferr := repo.ChangedFiles(ctx); ferr == nil && len(files) > 0 {
			rec.Session.ChangedFiles = files
	...
	// 12. Commit / open PR (real runs only).
	if !o.dryRun && repo != nil && (o.commit || o.openPR) {
		...
		if err := repo.Commit(ctx, commitMessage(o.task, rec.Session.ID)); err != nil {
			rec.Logf("commit: %v", err)
		}
		if o.openPR {
			out, perr := repo.OpenPR(ctx, git.PRInput{
				Title: "AgentBox: " + o.task, Body: prBody(o.task, ep, rec.Session.ID), Branch: branch, Base: "main",
			})
			switch {
			case perr != nil:
...
func prBody(task string, ep policy.EffectivePolicy, sessionID string) string {
...
`, task, ep.Agent.Default, engineLabel(ep), ep.EnforcedNetwork(), ep.PolicyPath, sessionID, ep.EnforcedNetwork())
}
