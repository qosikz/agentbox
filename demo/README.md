...
running and `andbo` on your `PATH` (or `ANDBO=/path/to/andbo`).
...
The hero story: Andbo injects a (fake) API key into the sandbox so the agent
can do real work against ONE allowed API, then proves the key has nowhere to go.

| Beat | Command | Point |
|------|---------|-------|
| 1 | `cat andbo.yaml` | One allowed domain + an injected credential. |
| 2 | `andbo exec '… $ANDBO_FAKE_API_KEY …'` | The key is really inside the sandbox (value never printed). |
| 3 | `andbo exec 'git ls-remote …github.com…'` | ✓ The one allowed API stays reachable. |
| 4 | `andbo exec 'git ls-remote …evil.example.com…'` | ✗ Exfil host **refused at the proxy** (403, fail closed). |
| 5 | `andbo session show latest \| grep 'BLOCKED egress'` | The attempt is in the **audit trail**. |
...
| Beat | Command | Point |
|------|---------|-------|
| 1 | `cat andbo.yaml` | Policy allows exactly **one** domain (`github.com`). |
| 2 | `andbo exec 'git ls-remote …github.com…'` | ✓ Allowed domain is reachable (exit 0). |
| 3 | `andbo exec 'getent hosts gitlab.com'` | DNS is **dead** — no resolution side channel (exit 2). |
| 4 | `andbo exec 'git ls-remote …gitlab.com…'` | ✗ Connection **fails closed** at the proxy (403, exit 128). |
| 5 | `andbo session show latest \| grep …` | Every attempt is in the **audit trail**. |
...
```bash
andbo --version >/dev/null   # ensure it's installed and on PATH
...
```bash
TYPE=0 HOLD=0 ANDBO="$PWD/bin/andbo" demo/exfil-demo.sh
```
...
```bash
export ANDBO="$PWD/bin/andbo"   # so the recorded run finds your build
...
```markdown
<p align="center"><img src="demo/egress.svg" alt="Andbo enforces network egress" width="800"></p>
```
...
Put it right under the tagline, above **## Add Andbo to your agent harness** —
...
- Re-run is idempotent: the temp workspace and all `andbo-net-*/ext-*`
